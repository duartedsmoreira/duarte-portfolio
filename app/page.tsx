...
export default function Portfolio() {
  ...
  const gallery = [
    { type: "image", src: "/gallery/sla-annotated.png", caption: "SLA KPIs with drill-through" },
    { type: "image", src: "/gallery/crm-migration-map.png", caption: "CRM data mapping & validation" },
    { type: "video", src: "/gallery/usage-journey-demo.mp4", caption: "User behavior flow demo (blurred)" },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 text-gray-900">
      ...
      <nav className="sticky top-0 z-40 backdrop-blur bg-white/70 border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-6 text-sm">
            ...
            <a href="#resume" className="hover:text-blue-700">CV</a>
      ...
      <header id="home" className="max-w-6xl mx-auto px-6 py-12 grid md:grid-cols-3 gap-8 items-center">
        <div className="md:col-span-2">
          ...
          <p className="text-gray-600">
            I turn messy data into clear decisions. I build reliable data models, actionable dashboards, and
            user-friendly analytics that improve processes and drive outcomes.
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            {skills.map((s) => (
              <span key={s} className="text-xs bg-white border border-gray-200 rounded-full px-3 py-1 shadow-sm">
                {s}
              </span>
            ))}
          </div>
          <div className="mt-6 flex gap-3">
            ...
            <Button asChild variant="secondary" className="rounded-2xl">
              <a href="/duarte-moreira-cv.pdf" download>
      ...
      <section id="resume" className="max-w-6xl mx-auto px-6 mt-12">
        ...
        <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="space-y-5">
              {experience.map((job, i) => (
                <div key={i} className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
                  ...
                  <p className="text-sm text-gray-700">{job.company} • {job.loc}</p>
          ...
          <div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm mb-5">
              ...
              <p className="text-sm text-gray-600">Setúbal Polytechnic Institute • 09/2023 – 09/2025</p>
            </div>
            <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm mb-5">
              <p className="font-medium">Bachelor’s in Information Systems Management</p>
              <p className="text-sm text-gray-600">Setúbal Polytechnic Institute • 09/2017 – 09/2021</p>
            </div>

            <h3 className="font-semibold mb-2">Languages</h3>
            <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm mb-5 text-sm text-gray-700">
              Portuguese (Native) • English (Proficient) • French (Advanced)
            </div>

            <div className="flex gap-3 flex-wrap">
              <Button asChild className="rounded-2xl">
                <a href="/duarte-moreira-cv.pdf" download><Download className="w-4 h-4 mr-2"/>Download CV</a>
              </Button>
              <Button asChild variant="secondary" className="rounded-2xl">
                <a href="/duarte-moreira-cv.pdf" target="_blank" rel="noreferrer">
                  <FileText className="w-4 h-4 mr-2"/>Open PDF
                </a>
              </Button>
            </div>
          </div>
        ...
        <div className="mt-6 bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
          <p className="text-sm text-gray-600 mb-3">Inline preview (place <code>duarte-moreira-cv.pdf</code> in your <code>public/</code> folder).</p>
            ...
            <div key={i} className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
              {g.type === "image" ? (
                <img
                  ...
                  src={g.src}
                  alt={g.caption}
                  onClick={() => setLightbox({ open: true, type: "image", src: g.src, caption: g.caption })}
                />
              ) : (
                <video
                  className="w-full h-48 object-cover cursor-pointer"
                  src={g.src}
                  onClick={() => setLightbox({ open: true, type: "video", src: g.src, caption: g.caption })}
                  controls
                />
              )}
              <div className="p-3 text-sm text-gray-700">{g.caption}</div>
            </div>
          ))}
        </div>
      </section>
